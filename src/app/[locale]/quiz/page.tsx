...
function ResultsScreen({
  ...
  useEffect(() => {
    trackEvent(sessionId, "quiz", "results_viewed", { personality_result: personalityKey }, locale);
  ...
  useEffect(() => {
    if (contactInfo?.type === "email" && !emailSent) {
      ...
      fetch("/api/send-results", {
        ...
        trackEvent(sessionId, "quiz", "results_email_sent", {}, locale);
      }).catch(() => { /* silent fail */ });
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  const sendMessage = async (text: string) => {
    ...
    setIsLoading(true);
    if (!chatStarted) {
      trackEvent(sessionId, "quiz", "chat_started", {}, locale);
    }
    trackEvent(sessionId, "quiz", "chat_message", { message_count: messages.length + 1 }, locale);
    setChatStarted(true);
      ...
      <div className="w-full lg:w-1/2 min-h-[100dvh] bg-off-white overflow-y-auto pb-24 lg:pb-0">
        ...
        <div className="px-4 md:px-12 lg:px-16 py-6 md:py-8">
          ...
          <Link
            ...
            onClick={() => trackEvent(sessionId, "quiz", "cta_clicked", { cta_url: ctaUrl }, locale)}
          >
